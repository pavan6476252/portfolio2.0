...
function ShowSpecificBlogScreen() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white dark:bg-gray-900 rounded-lg shadow-lg">
      ...
      />
      <h1 className="text-3xl dark:text-white font-bold mt-4">{data?.getBlogBySlug.metaTitle}</h1>
      <p className="text-gray-700 dark:text-gray-300 mt-2">{data?.getBlogBySlug.metaDescription}</p>
      <div className="flex items-center mt-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {new Date(parseInt(data?.getBlogBySlug?.createdAt??'0')).toLocaleDateString()}
          </p>
        </div>
      </div>
      <div className="mt-4">
      ...
      <div className="mt-4">
        ...
        <MarkdownEditor.Markdown source={data?.getBlogBySlug?.markdownContent} className="p-2" />
        {/* </div> */}
      </div>
      <div className="mt-8">
